    ...
    return (
      <main className="login-shell" dir="rtl">
        ...
        <section className="login-card">
          ...
          <div className="eyebrow">ALEPPO CENTER CASH <span>•</span> CLOUD ONLY</div>
          <h1>حساباتك، مرتّبة<br /><em>وبلا وجع راس.</em></h1>
          <p className="login-copy">دفتر حساباتك التجاري بمكان واحد. سريع، واضح، ومحفوظ على السحابة.</p>
  ...
  return (
    <main className="app-shell" dir="rtl">
      <aside className={`sidebar ${showMobileNav ? "open" : ""}`}>
        ...
        <div className="local-card"><div className="local-card-icon"><ShieldCheck size={17} /></div><div><strong>سحابي وآمن</strong><span>البيانات محفوظة في Supabase</span></div><Check size={16} className="check-icon" /></div>
...
function DashboardView({ accounts, totals, currency, onToggleCurrency, onOpenAccount, onAddPayment, onGoAccounts }: { accounts: Account[]; totals: Record<Currency, { credit: number; debit: number }>; currency: Currency; onToggleCurrency: () => void; onOpenAccount: (id: number) => void; onAddPayment: () => void; onGoAccounts: () => void }) {
  const todayLabel = new Intl.DateTimeFormat("ar-SY", { weekday: "long", day: "numeric", month: "long", year: "numeric" }).format(new Date());
